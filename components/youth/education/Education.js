...
const servicesList =  [ 'Colleges','Coaching Center','Homeschooling','Online Learning', 'Schools','Universities']
  ...
  const handleServiceSelection = async (selectedService) => {
    try {
      ...
      const businessSnapshot = await firestore()
        ...
        .where('categories', 'array-contains', selectedService)
        .get();
  
      const businessDetails = businessSnapshot.docs
        .map((doc) => doc.data())
        .filter((business) => business.categories.includes(selectedService));
